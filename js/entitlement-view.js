...
(function(exports) {
  ...
  function EntitlementView() {
    ...
    this.render = function($el) {
      ...
      if (state.selectedPlan !== null) {
        for (var i = 0; i < state.entitlementInfo.length; i++) {
          if (state.selectedPlan === state.entitlementInfo[i][0]) {
            planTitle = state.entitlementInfo[i][2];
          }
        }
        expireOn = new Date(state.planEndDate).toDateString();
      }
      ...
      if (this.$parentContainer !== $el) {
        this.$parentContainer = $el;
      } else {
        $('.app-container-buttons').remove();
      }
    ...
    this.renderDialog = function(data) {
      ...
      touches.registerTouchHandler('purchase-button', this.handleButtonTap);
    };
        ...
        for (var e = 0; e < entitlements.length; e++) {
          if (entitlements[e][0] === sku) {
            if (entitlements[e][1] === true) {
              ...
              $(btn)
                .find('.dialog-entitlement-price')
                .html(this.BUTTON_PURCHASED_MESSAGE);
            }
          }
        }
      }
    ...
    this.selectCurrentDialogItem = function() {
      ...
      $('.img-thumb-container-display').removeClass(
        'img-thumb-container-display'
      );
    ...
    var state = {
      entitlementInfo: [
        ['one.month.premium', false, 'Premium 1 Month', '30'],
        ['three.month.premium', false, 'Premium 3 Months', '90'],
        ['six.month.premium', false, 'Premium 6 Months', '180'],
        ['one.year.premium', false, 'Premium 12 Months', '360']
      ],
    ...
    this.handleOnPurchaseResponse = function(e) {
      if (e.purchaseRequestStatus === amzn_wa.IAP.PurchaseStatus.SUCCESSFUL) {
        ...
        // console.log(e);
        state.selectedPlan = e.receipt.sku;
        state.planStartDate = new Date();
        for (var i = 0; i < state.entitlementInfo.length; i++) {
          if (state.selectedPlan === state.entitlementInfo[i][0]) {
            var dt = state.planStartDate;
            state.planEndDate = dt.setDate(
              dt.getDate() + parseInt(state.entitlementInfo[i][3])
            );
          }
        }
        ...
        e.purchaseRequestStatus === amzn_wa.IAP.PurchaseStatus.ALREADY_ENTITLED
      ) {
        ...
        e.purchaseRequestStatus === amzn_wa.IAP.PurchaseStatus.FAILED
      ) {
        if (purchaseItemButtonPressed) {
          alert('We were unable to complete your purchase request');
        } else {
          console.log(
            'Purchase request from previous session returned a failure response'
          );
        }
      } else if (
        e.purchaseRequestStatus === amzn_wa.IAP.PurchaseStatus.INVALID_SKU
      ) {
        alert('Invalid SKU');
      }
      this.refreshPageState();
    ...
    this.handleOnPurchaseUpdatesResponse = function(e) {
      ...
      for (var i = 0; i < e.receipts.length; i++) {
        if (
          e.purchaseUpdatesRequestStatus ===
          amzn_wa.IAP.PurchaseUpdatesStatus.SUCCESSFUL
        ) {
          this.handleReceipt(e.receipts[i]);
        } else if (
          e.purchaseUpdatesRequestStatus ===
          amzn_wa.IAP.PurchaseUpdatesStatus.FAILED
        ) {
          alert('We were unable to complete your purchase request');
        }
      }
    ...
    this.purchaseItem = function(id) {
      if (amzn_wa.IAP === null) {
        alert(
          'You cannot buy this button, Amazon In-App-Purchasing works only with Apps from the Appstore.'
        );
      } else {
    ...
    this.initialize = function() {
      ...
      this.onSdkAvailable = function(resp) {
        ...
        amzn_wa.IAP.getPurchaseUpdates(
          state.lastPurchaseCheckTime !== null
            ? state.lastPurchaseCheckTime
            : amzn_wa.IAP.Offset.BEGINNING
        );
      };
      ...
      this.onItemDataResponse = function(data) {
        ...
        if (upgradeButtonPressed) {
          if (
            data.itemDataRequestStatus === amzn_wa.IAP.ItemDataStatus.SUCCESSFUL
          ) {
            this.renderDialog(data);
          } else if (
            data.itemDataRequestStatus === amzn_wa.IAP.ItemDataStatus.FAILED
          ) {
            alert('Failed to fetch items');
          } else if (
            data.itemDataRequestStatus ===
            amzn_wa.IAP.ItemDataStatus.SUCCESSFUL_WITH_UNAVAILABLE_SKUS
          ) {
