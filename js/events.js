  ...
  function Events(eventSet) {
    ...
    this.off = function(event, callback, context) {
      for (var evt in this.eventHandlers) {
        if (!event || event === evt) {
          this.eventHandlers[evt] = this.eventHandlers[evt].filter(
            this.createEventHandler(callback, context)
          );
        }
      }
    };
    ...
    this.createEventHandler = function(callback, context) {
      return function(element) {
        return (
          (callback && callback !== element.callback) ||
