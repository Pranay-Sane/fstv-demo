...
(function(exports) {
  ...
  function OneDView() {
    ...
    this.createButtonView = function(displayButtonsParam) {
      ...
      buttonView.render(
        this.$el.find('#summary-buttons-container'),
        buttonArr,
        buttonView.handleButtonCallback
      );
    };
    ...
    this.showExtraData = function(index) {
      ...
      this.$el
        .find('#summaryImage')
        .attr('src', this.rowElements[index].imgURL);
      this.$el.find('.one-D-summary-title').html(this.rowElements[index].title);
      this.$el
        .find('.one-D-summary-description')
        .html(this.rowElements[index].description);
      ...
      if (this.rowElements[index].type === 'video-live') {
        if (this.rowElements[index].isLiveNow) {
          ...
          this.$el
            .find('.one-D-summary-description')
            .html(
              '<div class="one-D-live-icon" ></div>' +
                '<div>' +
                this.rowElements[index].description +
                '</div>'
            );
          this.$el
            .find('.one-D-summary-description')
          ...
          this.$el
            .find('.one-D-summary-pubdate')
            .html(
              '<span class = "time-upcoming">' +
                this.rowElements[index].upcomingTime +
                '</span>'
            );
        ...
        if (this.rowElements[index].pubDate) {
          this.$el
            .find('.one-D-summary-pubdate')
            .html(this.rowElements[index].pubDate.toLocaleString());
        }
        this.$el.find('.one-D-summary-description').css('margin-top', '');
      }
    };
