...
(function(exports) {
  ...
  function PlaylistPlayerView(settings) {
    ...
    this.render = function($el, items, startIndex) {
      ...
      //touch events
      touches.registerTouchHandler(
        'player-content-video',
        this.handleTouchPlayer
      );
      touches.registerTouchHandler(
    ...
    this.handleTouchPlayer = function(e) {
      if (e.target.className === 'player-back-button') {
        //back button
    ...
    this.setUpNextPlayer = function() {
      ...
      if (this.items.length > this.nextIndex) {
        switch (this.items[this.nextIndex].source) {
          case 'youtube':
    ...
    this.handleVideoStatus = function(currentTime, duration, type) {
      if (type === 'playing') {
        if (this.$previewEl) {
          ...
          if (
            this.currentPlayerView.adPlaying &&
            this.currentPlayerView.adPlaying()
          ) {
            return;
    ...
    this.startNextVideo = function() {
      ...
      if (this.preloadedPlayerViewError) {
        this.trigger(
          'error',
          this.preloadedPlayerViewError.errType,
          this.preloadedPlayerViewError.errStack
        );
        this.preloadedPlayerViewError = null;
      }
      this.currentPlayerView.remove();
    ...
    this.handleControls = function(e) {
      if (this.currentView) {
        if (
          this.$previewEl &&
          this.$previewEl.is(':visible') &&
          !this.previewDismissed &&
          !this.seekAction(e.keyCode)
        ) {
          switch (e.keyCode) {
            case buttons.BACK:
            case buttons.DOWN:
          ...
          this.$previewEl &&
          !this.currentPlayerView.controlsCurrentlyShowing() &&
          !this.$previewEl.is(':visible') &&
          !this.previewDismissed
        ) {
          switch (e.keyCode) {
